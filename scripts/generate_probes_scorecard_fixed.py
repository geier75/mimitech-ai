#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fixed Scorecard with visible goal lines and wave markers"""
...
function sparkline(el, series) {{
  ...
  // Add goal lines FIRST (behind main line)
  const addGoalLine = (percent, color, label) => {{
    const y = sy(percent);
  ...
  // Add wave markers (vertical lines every 10k steps)
  if (maxX > minX) {{
    const start = Math.ceil(minX/10000)*10000;
    for (let step=start; step<=maxX; step+=10000) {{
      const x = sx(step);
      const line = document.createElementNS(svgNS, 'line');
      ...
      line.setAttribute('stroke', '#243042');
  ...
  // Add goal lines
  addGoalLine(80, '#2a6e4c', '80%');
  addGoalLine(65, '#6e662a', '65%');
  
  // Add main accuracy line
...
def main():
    ...
    def build_once():
        ...
        print(f"✅ Fixed scorecard: {out_html} • tasks={len(summary['tasks'])} • files={len(glob.glob(os.path.join(results_dir,'*.json')))}")
