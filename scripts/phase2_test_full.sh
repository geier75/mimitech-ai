...
with open(csv_path, newline='', encoding='utf-8') as f, open(out_path, 'w', encoding='utf-8') as w:
    ...
    for row in reader:
        ...
        for key, value in row.items():
            key_lower = key.lower()
            if key_lower in ['problem', 'question', 'input', 'query', 'prompt']:
                normalized_row['problem'] = value
            elif key_lower in ['solution', 'answer', 'output', 'response', 'completion']:
...
gate_and_stop_if_fail(){
  ...
  if [ "${SIMULATE}" = "1" ]; then
    ...
    # Generate slightly varying but passing scores
    local stats_score; stats_score=$(echo "scale=1; $STATS_T + $(shuf -i -30 -n 1 | head -1) / 10" | bc)
    local safety_score; safety_score=$(echo "scale=1; $SAFETY_T + $(shuf -i -20 -n 1 | head -1) / 10" | bc)
    local contam_score; contam_score=$(echo "scale=1; $CONTAM_T + $(shuf -i -50 -n 1 | head -1) / 10" | bc)
    
    # Ensure all pass
    stats_score=$(echo "if ($stats_score < $STATS_T) $STATS_T + 1.0 else $stats_score" | bc)
    safety_score=$(echo "if ($safety_score < $SAFETY_T) $SAFETY_T + 1.0 else $safety_score" | bc)
    contam_score=$(echo "if ($contam_score < $CONTAM_T) $CONTAM_T + 1.0 else $contam_score" | bc)
...
distill_merge(){
  ...
  log "🔄 Distilling and merging: $(basename "$adapter_ckpt") → $(basename "$base_ckpt")"
  
  if [ "${SIMULATE}" = "1" ]; then
    # Erzeuge „Merged"-Dummy aus Base + Adapter
    mkdir -p "$outdir"
    
    # Extract performance metrics from adapter
    local adapter_acc; adapter_acc=$(jq -r '.metrics.final_accuracy // 0.85' "$adapter_ckpt")
    local adapter_loss; adapter_loss=$(jq -r '.metrics.final_loss // 0.65' "$adapter_ckpt")
    ...
    jq -n \
      ...
      --argjson gen_acc "$(echo "$adapter_acc - 0.02" | bc)" \
      '{
...
freeze_bundle(){
  ...
  mkdir -p "$outdir"
  ( cd "$(dirname "$model")" && shasum -a 256 "$(basename "$model")" > SHA256SUMS.txt ) || true
  tar -czf "$outdir/freeze_bundle.tgz" \
    -C "$(dirname "$model")" "$(basename "$model")" SHA256SUMS.txt
  cp "$gates" "$outdir/"
