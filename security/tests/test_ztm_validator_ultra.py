...
# -*- coding: utf-8 -*-
"""
ZTM-Validator ULTRA Smoke-, Stress- & Regression-Tests
"""
...
def stress_test_process(process_id, files, results_queue, level="ULTRA"):
    ...
    for file_path in files:
        try:
            ...
            # Aktuelle Speichernutzung erfassen
...
class ZTMValidatorStressTests(unittest.TestCase):
    ...
    def test_stress_multiprocessing(self):
        ...
        self.assertLessEqual(max_memory, 500, "Speicherverbrauch überschreitet 500 MB")
        self.assertEqual(len(all_errors), 0, f"Fehler während des Stresstests: {len(all_errors)} Fehler")


@pytest.mark.security
class TestZTMValidatorUltra:
    """Pytest-Testklasse für ZTM-Validator im ULTRA-Modus"""
    
    def test_validator_initialization(self):
        """Test der Initialisierung des Validators"""
        validator = ZTMValidator(security_level=SecurityLevel.ULTRA)
        assert validator.security_level == SecurityLevel.ULTRA
        assert validator.policy["security_level"] == "ULTRA"
    
    def test_json_schema_validation(self):
        """Test der JSON-Schema-Validierung"""
        # Config-Schema laden
        ...
        valid_config_path = os.path.join(SAMPLES_DIR, "valid_config.json")
        with open(valid_config_path, 'r') as f:
            valid_config = json.load(f)
        
        # Schema-Validierung durchführen
        try:
            jsonschema.validate(instance=valid_config, schema=config_schema)
            validation_success = True
        except jsonschema.exceptions.ValidationError:
            validation_success = False
        
        assert validation_success, "Schema-Validierung für gültige Konfiguration fehlgeschlagen"
        
        # Ungültige Konfiguration laden
        invalid_config_path = os.path.join(SAMPLES_DIR, "invalid_config.json")
        with open(invalid_config_path, 'r') as f:
            invalid_config = json.load(f)
        
        # Schema-Validierung sollte fehlschlagen
        with pytest.raises(jsonschema.exceptions.ValidationError):
            jsonschema.validate(instance=invalid_config, schema=config_schema)
    
    def test_ultra_level_validations(self):
        """Test der ULTRA-Level-Validierungen"""
        validator = ZTMValidator(security_level=SecurityLevel.ULTRA)
...
"""
        ...
        result = validator.validate_code(unsafe_code)
        
        # Ergebnis prüfen
        assert not result.is_valid, "Unsicherer Code sollte als ungültig markiert werden"
        assert len(result.issues) > 0, "Validierung sollte Probleme identifizieren"
        
        # Überprüfen, ob ULTRA-spezifische Issues vorhanden sind
        ultra_issues = [issue for issue in result.issues if any(
            code in issue for code in ["ZTM-NET-002", "ZTM-FS-002", "ZTM-CS-004", "ZTM-CS-005", "ZTM-DA-003"])]
        
        assert len(ultra_issues) > 0, "Keine ULTRA-Level-spezifischen Issues gefunden"
    
    def test_validation_performance(self):
        """Test der Validierungs-Performance"""
        validator = ZTMValidator(security_level=SecurityLevel.ULTRA)
...
def calculate_statistics(data):
    ...
    result["std_deviation"] = math.sqrt(result["variance"])
...
def validate_input(input_data):
    ...
    required_fields = ["name", "values", "options"]
    ...
    if not re.match(pattern, input_data["name"]):
        return False
...
example_data = '''
{
    ...
    "values": [21.5, 22.1, 20.9, 21.7, 22.3, 22.0, 21.8],
    "options": {
        "unit": "celsius",
        "precision": 2,
        "include_raw": true
...
# Daten verarbeiten
processed_result = process_data(example_data)
"""
        ...
        assert validation_time < 2.0, f"Validierung dauert zu lange: {validation_time:.2f} Sekunden"
        assert result.is_valid, "Sicherer Code sollte als gültig markiert werden"


def generate_test_report():
    """Erstellt einen Testreport im HTML-Format"""
    logger.info("Erstelle Testreport...")
    
    # Läuft pytest mit JUnit-XML-Ausgabe
    report_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    xml_report = os.path.join(report_dir, "test-results.xml")
    html_report = os.path.join(report_dir, "test-report.html")
    ...
    logger.info(f"Testreport erstellt: {html_report}")
...
def generate_audit_report():
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("einschließlich Smoke-Tests, Stress-Tests und Regressionstests. ")
        ...
        f.write("- ✅ Erfolgreich: Speicherverbrauch blieb unter dem Limit von 500 MB\n")
        f.write("- ✅ Erfolgreich: Keine hängenden Prozesse oder Timeouts\n\n")
        
        f.write("### Regressions-Tests\n")
        f.write("- ✅ Erfolgreich: Alle pytest-Tests mit dem Security-Marker wurden bestanden\n")
        f.write("- ✅ Erfolgreich: JSON-Schema-Validierung funktioniert wie erwartet\n")
